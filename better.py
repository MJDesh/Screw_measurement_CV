...
ACCEPTED_FOLDER = os.path.join(OUTPUT_FOLDER, "accepted")
REJECTED_FOLDER = os.path.join(OUTPUT_FOLDER, "rejected")
os.makedirs(ACCEPTED_FOLDER, exist_ok=True)
os.makedirs(REJECTED_FOLDER, exist_ok=True)

CSV_FILE = os.path.join(OUTPUT_FOLDER, "measurements.csv")
...
def process_folder(folder, screw_type):
    ...
    for img_path in images:
        ...
        save_path = os.path.join(ACCEPTED_FOLDER if status == "ACCEPTED" else REJECTED_FOLDER, img_name)
        cv2.imwrite(save_path, output)

        with open(CSV_FILE, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([img_name, f"{length_mm:.2f}", f"{width_mm:.2f}", status, screw_type])
    ...
    try:
        ...
        print("✅ Dashboard script executed")
    except Exception as e:
        print("⚠ Could not run dashboard.py:", e)
...
def open_dashboard():
    dashboard_path = os.path.join(OUTPUT_FOLDER, "QC_Dashboard_Trend.png")
...
def open_accepted():
    os.startfile(ACCEPTED_FOLDER)


def open_rejected():
    os.startfile(REJECTED_FOLDER)
...
tk.Button(root, text="Run Inspection ✅",
          ...
          width=22, bg="lightgreen", font=("Arial", 12)).pack(pady=15)

tk.Button(root, text="📂 View Accepted Screws",
          command=open_accepted, width=22).pack(pady=3)

tk.Button(root, text="📂 View Rejected Screws",
          command=open_rejected, width=22).pack(pady=3)

tk.Button(root, text="📄 View CSV Report",
          command=open_csv, width=22).pack(pady=3)

tk.Button(root, text="📊 View Dashboard",
          command=open_dashboard, width=22, bg="orange").pack(pady=3)
